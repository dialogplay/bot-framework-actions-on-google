...
const EXIT_TRIGGERS = ["bye", "exit", "quit","さようなら","バイバイ","stop","ストップ","終了","キャンセル"];
const EXIT_MESSAGE = "また話しかけてね。";
let manager;
...
const sendMessageToBot = (app, bot, input, userId, state, user) => {
  ...
  let responder = event => {
    if (event.from.id !== app.getConversationId()) {
      ...
      console.log("beforeEnque response from bot " + CurrentTime())
  ...
  console.log("beforesendMessage " + CurrentTime())
...
let continueConversation = function (input, app, state, conversationTimeout, shouldGetUsersNameFromGoogle, user) {
  const conversationId = app.getConversationId().substr(0, 50);
  ...
  if (EXIT_TRIGGERS.includes(input.toLowerCase())) {
    ...
    app.tell(EXIT_MESSAGE + app.getUserName().familyName);
  } else {
    getDirectLineClient(state, conversationTimeout, conversationId)
      .then(bot => {
        ...
        if (shouldGetUsersNameFromGoogle && (!user || !app.getUserName())) {
          ...
          // Request permissions
          app.askForPermission('使用するためには', permission, state);
          ...
          console.log("beforesendMessageToBottimne " + CurrentTime())
...
const fulfill = (onUserSignedInHandlerProvider, conversationTimeout, shouldGetUsersNameFromGoogle) => (app) => {
  ...
  if (intent === app.StandardIntents.PERMISSION) {
    ...
    console.log("beforecontinueConversationtimne " + CurrentTime())
